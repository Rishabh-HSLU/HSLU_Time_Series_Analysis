...
def plot_ets(synthetic_df1, synthetic_df2, period = [], trend="add", seasonal="add", residual="add", freq="D", title: str = '', nsimulations=8 ) -> None:
    ...
    # Plot 1: Series 1
    # Observed data
    ax1.plot(synthetic_df1.index, synthetic_df1,
             color='blue',
             label='Observed', linewidth=1.5)
    # Fitted values
    ax1.plot(fit1.fittedvalues.index, fit1.fittedvalues,
             linestyle='--', color='green',
             label='Fitted (HW)', linewidth=1.5)

    # Simulations
    for col in simulations1.columns:
        ax1.plot(simulations1.index, simulations1[col],
                 linestyle='-', color='grey', alpha=0.05)

    # Forecast
    forecast1 = fit1.forecast(8)
    ax1.plot(forecast1.index, forecast1,
             linestyle='--', color='green',
             label=f"ETS({residual}, {trend}, {seasonal})", linewidth=1.5)
    ...
    # Plot 2: Series 2
    # Observed data
    ax2.plot(synthetic_df2.index, synthetic_df2,
             color='blue',
             label='Observed', linewidth=1.5)

    # Fitted values
    ax2.plot(fit2.fittedvalues.index, fit2.fittedvalues,
             linestyle='--', color='green',
             label='Fitted (HW)', linewidth=1.5)

    # Simulations
    for col in simulations2.columns:
        ax2.plot(simulations2.index, simulations2[col],
                 linestyle='-', color='grey', alpha=0.05)

    # Forecast
    forecast2 = fit2.forecast(8)
    ax2.plot(forecast2.index, forecast2,
             linestyle='--', color='green',
             label= f"ETS({residual}, {trend}, {seasonal})", linewidth=1.5)
